...
(defn parse-line
  [line]
  (let [[_ name sector-id checksum] (re-matches #" *([a-z\\-]+)([0-9]+)\[([a-z]+)\]" line)]
    {:name name
...
(defn frequencies
  [name]
  (reduce (fn [frequencies char]
            (case char
              \- frequencies
              (assoc frequencies char (inc (get frequencies char 0)))))
          {}
          name))
...
(defn valid-checksum?
  [{:keys [name sector-id checksum]}]
  (= checksum
     (subs (apply str (map (fn [v] (get v 0))
                           (reverse (sort (fn [[char-a freq-a] [char-b freq-b]]
                                            (let [c (compare freq-a freq-b)]
                                              (if (zero? c)
                                                (* -1 (compare char-a char-b))
                                                c)))
                                          (vec (frequencies name))))))
           0 5)))
...
(defn solve
  []
  (reduce (fn [sum {:keys [name sector-id checksum] :as line}]
            (if (valid-checksum? line)
              (+ sum sector-id)
              sum))
          0
          (parse-data raw-data)))
